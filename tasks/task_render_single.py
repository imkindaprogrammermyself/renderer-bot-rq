...
from utils.exception import VersionNotFoundError, ReadingError, RenderingError, UnsupportedBattleTypeError
...
def task_render_single(data: bytes, requester_id: int, logs=False, benny=False, doom=False):
    ...
    try:
        ...
        try:
            video_data = get_renderer(replay_data.version)(replay_data=replay_data, fps=retrieve_from_db('FPS'),
                                                           quality=retrieve_from_db('QUALITY'), logs=logs,
                                                           benny=benny,
                                                           doom=doom).start()
