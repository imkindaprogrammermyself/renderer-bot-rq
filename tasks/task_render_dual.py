...
from utils.exception import VersionNotFoundError, ReadingError, RenderingError, \
    UnsupportedBattleTypeError, MultipleReplaysError, ArenaIdMismatchError, NotEnoughReplaysError
    ...
    try:
        ...
        with BytesIO(data) as zip_data:
            ...
            if len(zip_obj.namelist()) > 2:
                raise MultipleReplaysError

            if len(zip_obj.namelist()) < 2:
                raise NotEnoughReplaysError
            ...
            if len(difference) != 0:
                not_found = " or ".join(
                    f"`{i}`, `{i.upper()}`" for i in difference)
            ...
            if replay_data_a.arena_id != replay_data_b.arena_id:
                raise ArenaIdMismatchError

            try:
                share = {}

                renderer_a = get_renderer(replay_data_a.version)(replay_data=replay_data_a, dual=True, as_enemy=False,
                                                                share=share)
                renderer_b = get_renderer(replay_data_b.version)(replay_data=replay_data_b, dual=True, as_enemy=True,
                                                                share=share)
                ...
                writer = write_frames(path=video_file.name, fps=30, macro_block_size=25,
                                    quality=9, size=(800, 850), pix_fmt_in="rgba")
                writer.send(None)

                for idx, (a, b) in enumerate(zip(renderer_a.generator(), renderer_b.generator())):
