...
class RenderSingle(BaseCog):
    ...
    async def _worker(self, ctx: Context, logs=False, benny=False, doom=False):
        ...
        with BytesIO() as buf:
            ...
            buf.seek(0)
            job: Job = queue.enqueue(task_render_single, args=(
                buf.read(), ctx.author.id, logs, benny, doom), failure_ttl=180, result_ttl=180, ttl=job_ttl)
